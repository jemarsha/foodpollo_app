    ...
    def get(url):
        ...
        if len(ingredients) != 0:
            ...
            for ingredient in ingredients:
                ...
                if str_ing and str_ing != "Add all ingredients to list":
                    str_ing = str_ing.rstrip().replace('\n', ' ')
                    str_ing = re.sub(r'\s+', ' ', str_ing)
        ...
        if len(steps) != 0:
            for step in steps:
                ...
                str_step = str_step.rstrip().replace('\n','')
                if str_step:
        ...
        if len(nutrition_data) != 0:
            for nutrient in nutrition_data:
                ...
                nut = nut.rstrip().replace('\n','')
                nut = re.sub(r'\s+','',nut)
                if nut:
                    data['nutrients'].append(nut.rstrip().replace('\n',''))
